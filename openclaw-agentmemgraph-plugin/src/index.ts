...
import { AgentMemGraphClient } from "./client.js";
import type { AgentMemGraphPluginConfig, ResolvedConfig } from "./config.js";
...
export { AgentMemGraphClient } from "./client.js";
export type { AgentMemGraphPluginConfig } from "./config.js";
...
function errorContent(err: unknown): { content: ContentBlock[] } {
  const agentMemGraphErr = err as AgentMemGraphError;
  if (agentMemGraphErr.statusCode) {
    return textContent(
      `AgentMemGraph error (${agentMemGraphErr.statusCode}): ${agentMemGraphErr.message}`,
    );
  }
  return textContent(
    `AgentMemGraph error: ${err instanceof Error ? err.message : String(err)}`,
  );
}
...
/**
 * Convert OpenClaw session messages into AgentMemGraph (observation, action) steps.
 *
 * An OpenClaw agentic turn looks like:
 *   user → assistant(toolCalls) → toolResult → assistant(toolCalls) → toolResult → assistant(stop)
 *
 * We unfold this into multiple AgentMemGraph steps:
 ...
 *
 * This gives AgentMemGraph's structuring pipeline (subgoal detection, reward eval,
...
async function autoRemember(
  ...
  try {
    ...
    console.error(
      `[agentmemgraph] auto-remember (${source}) failed:`,
...
export function createAgentMemGraphPlugin(config: AgentMemGraphPluginConfig): PluginEntry {
  const resolved = resolveConfig(config);
  const client = new AgentMemGraphClient(config);
  const defaultGraphId = resolved.defaultGraphId;

  // Captured from hook contexts so `agentmemgraph.remember` calls — which only
    ...
    activate(api: OpenClawPluginApi) {
      ...
      api.registerTool({
        ...
        description:
          ...
          "by the AgentMemGraph service into semantic, procedural, and episodic memories.",
...
function formatFailure(reason: unknown): string {
  const err = reason as AgentMemGraphError;
  if (err && typeof err.statusCode === "number") {
    return `AgentMemGraph error (${err.statusCode}): ${err.message}`;
  }
  return `AgentMemGraph error: ${reason instanceof Error ? reason.message : String(reason)}`;
}
...
export default createAgentMemGraphPlugin;
