...
export interface AgentMemGraphPluginConfig {
  /** AgentMemGraph service URL (e.g. http://localhost:8080) */
  baseUrl: string;
  ...
  /**
   * Extra graph IDs that are always included in `agentmemgraph.recall` fan-out.
   * Typically a user-level semantic graph shared across agents.
   * Writes (`agentmemgraph.remember`, auto-remember) never target these graphs.
   */
...
export function resolveConfig(raw: AgentMemGraphPluginConfig): ResolvedConfig {
  return {
