...
describe("OpenClaw plugin", () => {
  ...
  it("registers both tools", () => {
    ...
    expect(tools["agentmemgraph.remember"]).toBeDefined();
    expect(tools["agentmemgraph.recall"]).toBeDefined();
  });

  // ── agentmemgraph.remember ─────────────────────────────────────────────

  describe("agentmemgraph.remember", () => {
    it("stores semantic memory from free text", async () => {
      ...
      const result = await tools["agentmemgraph.remember"].execute("call-1", {
    ...
    it("stores trajectory from goal + steps", async () => {
      ...
      const result = await tools["agentmemgraph.remember"].execute("call-2", {
    ...
    it("returns guidance when nothing to store", async () => {
      ...
      const result = await tools["agentmemgraph.remember"].execute("call-3", {});
    ...
    it("requires graph_id when no default", async () => {
      ...
      const result = await tools["agentmemgraph.remember"].execute("call-4", {
        text: "some fact",
      });
    ...
    it("uses explicit graph_id over default", async () => {
      ...
      await tools["agentmemgraph.remember"].execute("call-5", {
  ...
  describe("agentmemgraph.recall", () => {
    it("returns LLM-synthesized reasoning by default", async () => {
      ...
      const result = await tools["agentmemgraph.recall"].execute("call-6", {
    ...
    it("returns raw retrieval prompt when raw=true", async () => {
      ...
      const result = await tools["agentmemgraph.recall"].execute("call-7", {
    ...
    it("passes mode through to the API", async () => {
      ...
      await tools["agentmemgraph.recall"].execute("call-8", {
    ...
    it("handles API errors gracefully", async () => {
      ...
      const result = await tools["agentmemgraph.recall"].execute("call-9", {
        observation: "anything",
      });

      const text = getText(result);
      expect(text).toContain("AgentMemGraph error (404)");
    });
  });

  // ── Shared-read fan-out ──────────────────────────────────────────

  describe("agentmemgraph.recall with sharedReadGraphIds", () => {
    it("fans out reason across primary + shared graphs", async () => {
      ...
      const result = await tools["agentmemgraph.recall"].execute("call-fan-1", {
    ...
    it("fans out retrieve when raw=true", async () => {
      ...
      const result = await tools["agentmemgraph.recall"].execute("call-fan-2", {
        observation: "context?",
        raw: true,
      });
    ...
    it("returns partial results when a shared graph fails", async () => {
      ...
      const result = await tools["agentmemgraph.recall"].execute("call-fan-3", {
        observation: "anything",
      });
    ...
    it("dedupes when primary graph also appears in shared list", async () => {
      ...
      await tools["agentmemgraph.recall"].execute("call-fan-4", {
        observation: "x",
      });
    ...
    it("preserves single-graph output shape when sharedReadGraphIds is empty", async () => {
      ...
      const result = await tools["agentmemgraph.recall"].execute("call-fan-5", {
        observation: "x",
      });
  ...
  describe("auto-remember hooks", () => {
    ...
    it("sends trajectory to AgentMemGraph on before_reset", async () => {
      const { hooks } = activatePlugin("g1");
      mockFetch.mockResolvedValueOnce(
    ...
    it("sends trajectory to AgentMemGraph on before_compaction", async () => {
      const { hooks } = activatePlugin("g1");
      mockFetch.mockResolvedValueOnce(
        jsonResponse({ status: "ok", stats: {} }),
      );
    ...
    it("does not throw on API failure (best-effort)", async () => {
      ...
      expect(consoleSpy).toHaveBeenCalledWith(
        expect.stringContaining("[agentmemgraph] auto-remember"),
        expect.any(String),
      );
