...
import { AgentMemGraphClient } from "../src/client.js";
import { AgentMemGraphError, AgentMemGraphConnectionError } from "../src/types.js";
...
describe("AgentMemGraphClient", () => {
  let client: AgentMemGraphClient;

  beforeEach(() => {
    mockFetch.mockReset();
    client = new AgentMemGraphClient({
  ...
  it("omits API key header when not configured", async () => {
    const noAuthClient = new AgentMemGraphClient({ baseUrl: "http://localhost:8080" });
    mockFetch.mockResolvedValueOnce(jsonResponse({ graphs: [] }));
  ...
  describe("error handling", () => {
    it("throws AgentMemGraphError on 404", async () => {
      mockFetch.mockResolvedValueOnce(
      ...
      try {
        ...
        expect(err).toBeInstanceOf(AgentMemGraphError);
        expect((err as AgentMemGraphError).statusCode).toBe(404);
        expect((err as AgentMemGraphError).body).toEqual({ detail: "Graph not found" });
      }
    });
    ...
    it("throws AgentMemGraphConnectionError after exhausting retries", async () => {
      mockFetch
        .mockRejectedValueOnce(new TypeError("fetch failed"))
        .mockRejectedValueOnce(new TypeError("fetch failed"));

      await expect(client.listGraphs()).rejects.toThrow(
        AgentMemGraphConnectionError,
      );
    });
    ...
    it("does not retry on 400", async () => {
      ...
      await expect(client.createGraph()).rejects.toThrow(AgentMemGraphError);
  ...
  it("strips trailing slashes from baseUrl", async () => {
    const c = new AgentMemGraphClient({ baseUrl: "http://localhost:8080///" });
    mockFetch.mockResolvedValueOnce(jsonResponse({ graphs: [] }));
