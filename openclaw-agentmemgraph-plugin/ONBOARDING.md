# AgentMemGraph for OpenClaw — Onboarding

Long-term memory for your OpenClaw agent. Installs as a plugin, auto-saves
session trajectories, and gives the agent two tools: `agentmemgraph.remember` and
`agentmemgraph.recall`. This doc takes you from zero to a working memory loop in
about 10 minutes.

## What you'll have when you're done

- A AgentMemGraph service running at `http://localhost:8080`.
- An OpenClaw agent that:
  - Exposes `agentmemgraph.remember` / `agentmemgraph.recall` tools.
...
## 1. Start the AgentMemGraph service
...
export AGENTMEMGRAPH_API_KEY="dev-key-change-me"
```
...
```bash
source .env
uv run uvicorn agentmemgraph.api.app:app --host 0.0.0.0 --port 8080
```
...
```ts
import { createAgentMemGraphPlugin } from "@agentmemgraph/openclaw-plugin";

const plugin = createAgentMemGraphPlugin({
  baseUrl: "http://localhost:8080",
  apiKey: "dev-key-change-me",      // must match AGENTMEMGRAPH_API_KEY
...
| `apiKey` | none | Sent as `X-API-Key`. Required if `AGENTMEMGRAPH_API_KEY` is set on the server. |
...
```
Use agentmemgraph.remember with text="The user prefers meetings in the morning" and tags=["user-pref"]
```

Then in the same session (or a new one):

```
Use agentmemgraph.recall with observation="When does the user like to meet?"
```
...
- **Trajectory** — `goal` + `steps[{observation, action}]`. A full
  agent run. AgentMemGraph's structuring pipeline will extract semantic,
  procedural, and episodic memories from it.

### `agentmemgraph.recall`
...
3. To populate `user-facts`, call `agentmemgraph.remember` with an explicit
   `graph_id`:
   ```
   agentmemgraph.remember(graph_id="user-facts", text="User prefers async standups", tags=["user-pref"])
   ```
...
Writes (`agentmemgraph.remember` without an explicit `graph_id`, auto-remember
...
| `agentmemgraph.remember` returns `404` | Graph doesn't exist. Run the `POST /graphs` call from step 2. |
| `agentmemgraph.remember` returns `401` | `apiKey` in plugin config doesn't match server's `AGENTMEMGRAPH_API_KEY`. |
| Auto-remember never fires | `defaultGraphId` not set, or session had `< minSteps` turns. Check server stderr for `[agentmemgraph] auto-remember` lines. |
...
- API reference: every route lives under `agentmemgraph/api/routes/` — one file
  per resource (`graphs.py`, `memories.py`, `retrieval.py`).
- Environment variables, architecture, and limitations: see the repository
  root `README.md`.
