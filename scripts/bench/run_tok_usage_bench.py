"""Token-cost benchmark: AgentMemGraph vs OpenClaw-default memory replay.
...
Systems:
    ...
    B   AgentMemGraph trajectory-mode: structured on insert, recall via /reason
    C   AgentMemGraph semantic-mode:   pre-distilled facts on insert, recall via /reason
...
from agentmemgraph.clients.embedding import EmbeddingClient
...
def run_system_b(
    traj: Trajectory,
    queries: List[Query],
    graph,  # agentmemgraph.core.memory_graph.MemoryGraph
    exposed_llm: InstrumentedLLMClient,
) -> List[Tuple[str, str]]:
    """AgentMemGraph trajectory-mode. Pay structuring once, then /reason per query.
    ...
    """
    # --- INSERT (structuring) ------------------------------------------------
    from agentmemgraph.core.memory import Memory
...
def run_system_c(
    ...
    """AgentMemGraph semantic-mode. Pre-distilled facts inserted directly — no structuring.
...
def build_clients(bench_logger: BenchLogger):
    ...
    from agentmemgraph.clients.embedding import HTTPEmbeddingClient
...
def fresh_graph(internal_llm, embedder):
    ...
    import chromadb
    from agentmemgraph.clients.embedding import AgentMemGraphEmbeddingFunction
    from agentmemgraph.storage.chroma import ChromaStorage
    from agentmemgraph.graph_manager import GraphManager

    client = chromadb.EphemeralClient()
    storage = ChromaStorage(
        client=client,
        embedding_function=AgentMemGraphEmbeddingFunction(embedder),
