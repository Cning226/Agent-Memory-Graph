...
Reads --in (JSONL of per-call records) and produces:
  ...
  3. Crossover R* for tokens and dollars: smallest R at which a AgentMemGraph
...
def load_pricing(path: Path) -> Dict[str, Dict[str, float]]:
    ...
    import yaml  # PyYAML is already a transitive dep via agentmemgraph.
...
def render_report(
    ...
    # ---- Crossover summary ----
    lines.append("\n## Crossover R\\* (AgentMemGraph cum-cost <= OpenClaw-default cum-cost)\n")
