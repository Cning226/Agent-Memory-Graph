# Token-cost benchmark — AgentMemGraph vs OpenClaw default

A controlled experiment to measure where LLM tokens are spent when an agent
uses AgentMemGraph instead of OpenClaw's default replay-the-whole-transcript memory.

The point isn't just *fewer tokens overall* — the point is that AgentMemGraph
...
| `B` | **AgentMemGraph trajectory-mode** | structuring (`INTERNAL`) | classify + reason-synthesis (`INTERNAL`) + compact recall (`EXPOSED`) |
| `C` | **AgentMemGraph semantic-mode** | only embeddings (`INTERNAL_EMBED`) | same as B |
...
The bench reads the same env vars as the rest of AgentMemGraph, plus optional
...
`(trajectory, system)`) so caches never flatter AgentMemGraph's numbers.
...
- crossover R\* (smallest R where AgentMemGraph cum-cost ≤ baseline) — both
...
the experiment design says efficiency numbers are meaningless if AgentMemGraph
...
- **Not a real-OpenClaw integration test.** The "OpenClaw default" baseline
  ...
  with and without the `openclaw-agentmemgraph-plugin`.
