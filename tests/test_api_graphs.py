...
def test_browse_nodes_serializes_semantic_and_procedural_fields(client):
    client.post("/api/v1/graphs", json={"graph_id": "browse_fields"})
    insert = client.post(
        "/api/v1/graphs/browse_fields/memories",
        json={
            "mode": "structured",
            "semantic": [
                {"semantic_memory": "User prefers mornings", "tags": ["preference"]}
            ],
            "procedural": [
                {
                    "subgoal": "deploy",
                    "procedural_memory": "Run the deployment command",
                }
            ],
        },
    )
    ...
    semantic = client.get(
        "/api/v1/graphs/browse_fields/nodes?node_type=semantic"
    )
    assert semantic.status_code == 200
    assert semantic.json()["nodes"][0]["credibility"] == 10

    procedural = client.get(
        "/api/v1/graphs/browse_fields/nodes?node_type=procedural"
    )
    assert procedural.status_code == 200
    assert procedural.json()["nodes"][0]["subgoals"] == ["deploy"]
