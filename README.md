...
> 当前版本定位为个人维护的工程化重构项目。仓库不展示缺少原始结果文件支撑的领先指标或 Token 降幅数字；复现实验应同时公开配置、数据切分和原始输出。
...
- FastAPI 路由内部仍使用同步 LLM 和 Embedding 客户端。
- 尚未实现分布式后台任务队列和独立 Worker。
- 默认记忆排序主要依据向量相关性。
- 核心检索尚未实现通用的多跳图邻接扩展。
- 本地大模型服务需要较高显存，CPU 环境更适合连接远程 API。
- 三套评测代码仍带有研究原型特征，后续会逐步统一到同一评测 CLI。
...
项目按照 [Apache License 2.0](LICENSE) 发布。本仓库属于对既有研究代码的重命名与工程化重构；修改范围和来源说明记录在 [NOTICE](NOTICE) 中。个人维护、重构和新增内容不改变原始代码应保留的版权与许可义务。
