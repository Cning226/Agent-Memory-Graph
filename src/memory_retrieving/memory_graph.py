...
class MemoryGraph:
    ...
    def retrieve_episodic_nodes(self, observation: str=None):
        ...
        '''merge episodic from same session when session ids exist'''
        vote_session = {}
        sem_session = {}

        for semantic_node in semantic_nodes:
            session_id = getattr(semantic_node, "session_id", None)

            if session_id is None:
                for episodic_node in getattr(semantic_node, "episodic_nodes", []):
                    session_id = getattr(episodic_node, "session_id", None)
                    if session_id is not None:
                        break

            sem_session[semantic_node.semantic_id] = session_id

            if session_id is not None:
                vote_session[session_id] = vote_session.get(session_id, 0) + 1

        cnt = 0
        used_session_ids = set()

        for session_id, value in vote_session.items():
            if value >= 3:
                episodic_memory_str += f"Relevant Memory {cnt}:\n{self.get_session_memory(session_id)}"
                used_session_ids.add(session_id)
                cnt += 1

        for semantic_node in semantic_nodes:
            session_id = sem_session.get(semantic_node.semantic_id)

            if session_id in used_session_ids:
                continue

            episodic_nodes = getattr(semantic_node, "episodic_nodes", [])
            if episodic_nodes:
                # episodic_text = "\n".join(
                #     episodic_node.get_episodic_memory()
                #     for episodic_node in episodic_nodes
                # )
                # episodic_memory_str += f"Relevant Memory {cnt}:\n{episodic_text}\n"
                episodic_texts = []
                ...
                for episodic_node in episodic_nodes:
                    try:
                        text = episodic_node.get_episodic_memory()
                    except Exception:
                        text = ""
    ...
    def retrieve_memory(self, goal: str=None, subgoal: str=None, state: str=None, observation: str=None, prompt_template: PromptBase=None, time: str="", task_type: str="", mode: str=None):
        ...
        if mode == None:
            ...
            if isinstance(mode, str):
                ...
                if "episodic" in mode_lower:
                    mode = "episodic_memory"
                elif "semantic" in mode_lower:
                    mode = "semantic_memory"
                elif "procedural" in mode_lower:
                    mode = "procedural_memory"
                else:
                    # 如果仍不合法，可默认 episodic_memory 或报错
                    mode = "episodic_memory"
        ...
        if mode == "episodic_memory":
            ...
            information = semantic_memory_str
        elif mode == "procedural_memory":
            information = procedural_memory_str
        else:
            information = ""
