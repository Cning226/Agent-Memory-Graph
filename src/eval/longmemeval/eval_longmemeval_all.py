...
#     response = wrapper_call_model(model_name="qwen3.5-27b", messages=messages)#7
...
#     )
#     response = wrapper_call_model(model_name="qwen3.5-27b", messages=[
...
def _build_memory_from_session(session, time):
    ...
    for turn in session[st:]:
        if turn["role"] == "assistant":
            ...
            if action is None:
                raise ValueError(
                    "Encountered user turn before any assistant action in session."
                )
            memory.append(
                action_t0=action,
                observation_t1=f"User Say: {turn['content']}",
...
worker_count = int(
    os.getenv("LONGMEMEVAL_SESSION_WORKERS", max(os.cpu_count() or 1, 1))
)
...
for n in range(500):
    ...
    cache_dir = os.path.abspath("../../../data_longmemeval/mg_cache")
    os.makedirs(cache_dir, exist_ok=True)
    cache_path = os.path.join(cache_dir, f"mg_cache_{question_id}.pkl")

    if os.path.exists(cache_path):
        print(f"Loading MG cache from {cache_path}")
        with open(cache_path, "rb") as f:
            mg = pickle.load(f)
        print("MG cache loaded")
    else:
        mg = MemoryGraph(
            tag_equal=TagEqual(),
            tag_relevant=TagRelevant(),
        ...
        print(
            f"Loading test {question_id} with {len(sessions)} sessions using {worker_count} workers"
        )
        with ThreadPoolExecutor(max_workers=worker_count) as executor:
            memories = list(executor.map(_build_memory_from_session, sessions, times))
        ...
        print(f"Saved MG cache to {cache_path}")
    ...
    messages, memory_map, sel_type = mg.retrieve_memory(
        goal=goal,
        observation=question,
        time=f"Date: {test['question_date']}",
        task_type=task_type,
    )

    memory_str = memory_map[sel_type]

    response = wrapper_call_model(
        model_name=os.environ.get("LLM_NAME", "qwen3.5-35b-a3b"),
        messages=messages,
    )
